...
export async function seedDatabase({ force = false }: { force?: boolean } = {}): Promise<SeedReport> {
  ...
  if (force || (await count('service')) === 0) {
    const services: Record<string, unknown>[] = [
      ...
      {
        slug: 'sample-photography',
        division: 'media',
        title: 'Photography',
        summary: 'Event, portrait and product stills — shot with the edit in mind, delivered organised.',
        description: 'Placeholder service. Replace with your actual packages: hours, number of edited frames, print rights and turnaround.',
        icon: 'image',
        bullets: ['Events, portraits, products', 'Colour-managed delivery', 'Private online gallery'],
        deliverables: ['Curated edited set', 'Web and print sizes', 'Online gallery'],
      ...
      {
        slug: 'sample-social-content',
        division: 'media',
        title: 'Social media & short-form',
        summary: 'Vertical edits built for TikTok, Reels and Shorts — captioned, cut fast, on-brand.',
        description: 'Placeholder service. Replace with retainer shapes, per-video pricing or content-day packages you actually offer.',
        icon: 'sparkle',
        bullets: ['Vertical 9:16 masters', 'Captions and hooks', 'Batch content days available'],
        deliverables: ['Vertical masters', 'Captioned versions', 'Thumbnail covers'],
      ...
      {
        slug: 'sample-live-streaming',
        division: 'media',
        title: 'Live streaming',
        summary: 'Multi-camera streams for services, conventions and launches — with a recording you keep.',
        description: 'Placeholder service. Replace with platforms you stream to, redundancy setup and pricing per day.',
        icon: 'gauge',
        bullets: ['Multi-camera switching', 'YouTube / Facebook / custom RTMP', 'Local recording as backup'],
        deliverables: ['Live stream', 'Multi-cam recording', 'Post-event cut (optional)'],
        tools: ['OBS', 'Atem Mini'],
        is_sample: true,
        status: 'published',
        sort_order: 7,
      ...
      {
        slug: 'sample-church-convention',
        division: 'media',
        title: 'Church & convention coverage',
        summary: 'Services, crusades and conventions covered respectfully — from the back row, not the front.',
        description: 'Placeholder service. Replace with how you handle multi-day programmes, choir/ministry sessions and repeat bookings.',
        icon: 'users',
        bullets: ['Multi-day programmes', 'Session-by-session delivery', 'Recurring booking rates'],
        deliverables: ['Session recordings', 'Highlight reel', 'Social cutdowns'],
        is_sample: true,
        status: 'published',
        sort_order: 8,
      },
  ...
  if (force || (await count('media_video')) === 0) {
    const clips = [
      ...
      { id: 'eRsGyueVLvQ', title: 'Sample cutdown — Sintel (vertical)', form: 'short_form', duration: 59 },
      { id: 'R6MlUcmOul8', title: 'Sample cutdown — Tears of Steel (vertical)', form: 'short_form', duration: 47 },
      { id: 'aqz-KE-bpKQ', title: 'Sample teaser — Big Buck Bunny (vertical)', form: 'short_form', duration: 38 },
      { id: 'TLK3A0RELQ3g', title: 'Sample teaser — Elephants Dream (vertical)', form: 'short_form', duration: 41 },
    ];
    let order = 0;
  ...
  if (force || (await count('project')) === 0) {
    const projects: Record<string, unknown>[] = [
      ...
      },
      {
        ...
        division: 'media',
        title: 'Sample — YouTube thumbnail set',
        category: 'thumbnail_design',
        form: null,
        summary: 'Placeholder cover set showing how thumbnail work renders on the portfolio.',
        role: 'Design, retouch',
        problem: 'Replace with the channel and what the covers needed to do.',
        solution: 'Replace with the design approach — type, colour, faces, and the click test.',
        services: ['sample-post-production'],
        tools: ['Photoshop', 'Affinity Photo'],
        deliverables: ['Twelve covers', 'Editable template'],
        metrics: [],
        is_sample: true,
        status: 'published',
        published_at: new Date().toISOString(),
      ...
      },
      {
        ...
        division: 'media',
        title: 'Sample — sermon series covers',
        category: 'thumbnail_design',
        form: null,
        summary: 'Placeholder series artwork for church media — title cards and matching social covers.',
        role: 'Design',
        problem: 'Replace with the series theme and the formats needed.',
        solution: 'Replace with how the artwork system was built to stretch across formats.',
        services: ['sample-post-production'],
        tools: ['Photoshop', 'After Effects'],
        deliverables: ['Series title cards', 'Social cover set'],
        metrics: [],
        is_sample: true,
        status: 'published',
        published_at: new Date().toISOString(),
      ...
      },
      {
        ...
        division: 'media',
        title: 'Sample — product launch covers',
        category: 'thumbnail_design',
        form: null,
        summary: 'Placeholder e-commerce cover set: hero crops and vertical variants for ads.',
        role: 'Design, retouch',
        problem: 'Replace with the product and the campaign goal.',
        solution: 'Replace with the layout system that kept every crop on-brand.',
        services: ['sample-post-production'],
        tools: ['Photoshop'],
        deliverables: ['Hero covers', 'Vertical ad variants'],
        metrics: [],
        is_sample: true,
        status: 'published',
        published_at: new Date().toISOString(),
    ...
    const pages: { slug: string; title: string; navLabel: string; surface: string; description: string; blocks: { type: string; name: string; props?: Record<string, unknown> }[] }[] = [
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'featured_work', name: 'Featured work', props: { limit: 4 } },
          { type: 'short_form_rail', name: 'Short-form rail', props: { limit: 8 } },
          { type: 'thumbnail_wall', name: 'Thumbnail designs', props: { limit: 3 } },
          { type: 'photo_gallery', name: 'Photography', props: { limit: 8 } },
          { type: 'service_grid', name: 'Services', props: { division: 'media', limit: 6, layout: 'list' } },
          { type: 'process_timeline', name: 'Process', props: {} },
          { type: 'about_split', name: 'About', props: {} },
