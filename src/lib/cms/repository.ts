...
export async function dashboardCounts(): Promise<Record<string, number>> {
  ...
  const queries: [string, string][] = [
    ...
    // ── section counters (Overview, and the three section hubs) ────────────────
    ['published_videos', "SELECT count(*)::int AS n FROM media_video WHERE status = 'published'"],
    ['hero_preview', "SELECT count(*)::int AS n FROM media_video WHERE hero_preview = TRUE AND status = 'published'"],
    ['video_stories', "SELECT count(*)::int AS n FROM media_video WHERE story_quote IS NOT NULL AND story_quote <> ''"],
    ['media_stories', "SELECT count(*)::int AS n FROM testimonial WHERE division = 'media'"],
    ['photo_galleries', "SELECT count(*)::int AS n FROM gallery WHERE division = 'media' AND kind = 'photo'"],
    [
      'photo_frames',
      "SELECT COALESCE(sum(CASE WHEN jsonb_typeof(items) = 'array' THEN jsonb_array_length(items) ELSE 0 END), 0)::int AS n FROM gallery WHERE division = 'media' AND kind = 'photo'",
    ],
    ['galleries', 'SELECT count(*)::int AS n FROM gallery'],
    ['skills', 'SELECT count(*)::int AS n FROM skill'],
    ['experience', 'SELECT count(*)::int AS n FROM experience_item'],
    ['certifications', 'SELECT count(*)::int AS n FROM certification'],
    ['resumes', 'SELECT count(*)::int AS n FROM resume_version'],
    ['active_resume', 'SELECT count(*)::int AS n FROM resume_version WHERE is_active = TRUE'],
    ['pages', 'SELECT count(*)::int AS n FROM page'],
    ['blocks', 'SELECT count(*)::int AS n FROM content_block'],
    ['navigation', 'SELECT count(*)::int AS n FROM navigation_item'],
    ['team', 'SELECT count(*)::int AS n FROM team_member'],
    ['socials', 'SELECT count(*)::int AS n FROM social_link'],
    ['verified_socials', 'SELECT count(*)::int AS n FROM social_link WHERE is_verified = TRUE'],
    ['seo_records', 'SELECT count(*)::int AS n FROM seo_record'],
  ];
