...
export const BLOCK_TYPES: BlockTypeDef[] = [
  ...
  },
  {
    type: 'featured_work',
    label: 'Featured work',
    description: 'One lead film panel plus secondary cards — the hierarchy section between hero and archive.',
    surfaces: ['media'],
    icon: 'film',
    propFields: [
      num('limit', 'Projects (lead + cards)', { min: 1, max: 7, default: 4 }),
      bool('featuredOnly', 'Featured projects only', { default: true, help: 'Off → falls back to the newest published work.' }),
      text('ctaLabel', 'Footer link label', { maxLength: 40 }),
      text('ctaHref', 'Footer link href', { maxLength: 120 }),
    ],
  },
  {
    type: 'short_form_rail',
    label: 'Short-form rail',
    description: 'Vertical TikTok / Reels / Shorts edits on a swipeable rail. Poster-only until tapped.',
    surfaces: ['media'],
    icon: 'play',
    propFields: [
      { key: 'videoIds', label: 'Picked videos', type: 'video', multiple: true, help: 'Leave empty to pull the latest published short-form videos.' },
      num('limit', 'Items', { min: 1, max: 16, default: 8 }),
      text('ctaLabel', 'Footer link label', { maxLength: 40 }),
      text('ctaHref', 'Footer link href', { maxLength: 120 }),
    ],
  },
  {
    type: 'photo_gallery',
    label: 'Photo gallery',
    description: 'Photography showcase — an editorial mosaic from a curated gallery or picked images.',
    surfaces: ['media'],
    icon: 'camera',
    propFields: [
      text('gallerySlug', 'Gallery slug (optional)', { maxLength: 80, help: 'Leave blank to use the latest photo gallery.' }),
      { key: 'images', label: 'Picked images', type: 'image', multiple: true, help: 'Overrides the gallery when set.' },
      { key: 'kind', label: 'Gallery kind', type: 'select', width: 'third', default: 'photo', options: [{ value: 'photo', label: 'Photo sets' }, { value: 'mixed', label: 'Mixed sets' }] },
      num('limit', 'Items', { min: 1, max: 16, default: 8 }),
      layout([{ value: 'editorial', label: 'Editorial mosaic' }, { value: 'grid', label: 'Even grid' }]),
      bool('showCaptions', 'Show captions in the viewer', { default: true }),
      text('caption', 'Section footer caption', { maxLength: 140 }),
    ],
  },
  {
    type: 'thumbnail_wall',
    label: 'Thumbnail / graphic wall',
    description: 'Thumbnail designs and cover art shown as designed covers — from a project category or picked images.',
    surfaces: ['media'],
    icon: 'image',
    propFields: [
      { key: 'category', label: 'Project category', type: 'select', width: 'third', default: 'thumbnail_design', options: MEDIA_CATEGORIES.map((c) => ({ value: c.value, label: c.label })) },
      { key: 'images', label: 'Picked images', type: 'image', multiple: true, help: 'Used when no projects match the category.' },
      num('limit', 'Items', { min: 1, max: 12, default: 6 }),
      text('caption', 'Section footer caption', { maxLength: 140 }),
    ],
  },
  {
