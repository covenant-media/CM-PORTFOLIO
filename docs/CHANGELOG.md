...
### Added
- **2026-09-10 — Media Portfolio redesign pass (cinematic, video-first).** Four new CMS blocks give the Media surface its editorial rhythm, all registered in the block catalogue (composer + renderer + plans stay in sync, guarded by the registry test):
  - `featured_work` — hierarchy section: one lead film panel (21:10 cinematic crop on desktop, poster + "Play reel (N)" lightbox control + View-project CTA) plus 2–3 secondary cards. Featured projects first, topping up with the newest published work when fewer than two are featured.
  - `short_form_rail` — vertical TikTok/Reels/Shorts showcase on a swipeable snap rail (`ui/Rail.tsx`, new client primitive: native scroll + arrow buttons, edge fades, reduced-motion aware). Poster-only tiles; tap opens the shared lightbox with sound.
  - `photo_gallery` — photography showcase as an editorial mosaic (lead image spans 2×2) or even grid, from a curated gallery slug or picked images; lightbox with captions.
  - `thumbnail_wall` — thumbnail/cover design work rendered as designed covers (16:9, bold overlay title, accent underline on hover) from the `thumbnail_design` project category or picked images.
  - All four render honest empty states with CMS guidance, `is_sample` badges, lazy posters, and zero third-party iframes until interaction.
- **Media homepage structure** (plan fallback + seed composition + fill-content copy): hero ("WE CAPTURE. WE CREATE. WE INSPIRE." tagline headline, location + availability meta strip from Site settings) → featured work → short-form rail → thumbnail wall → photo gallery → services (editorial list) → process (Discovery → Planning → Production → Edit → Delivery, CMS-editable) → about (portrait attached) → testimonials → contact. Mobile now always gets a swipeable hero preview strip (previously the floating-card variant hid all previews below `lg`).
- **`tools_grid` is now source-aware** (prop: manual/services/projects) and division-aware: on Media pages with no manual list it aggregates `service.tools` (Sony FX3, DaVinci Resolve, Lightroom, OBS…); Tech keeps the skills-evidence aggregation. `/media/services` gained a tools grid seeded from services.
- **Project detail (media + tech)**: additive "Services" and "Toolkit" chip rows render next to "Stack" when the fields are filled — hidden entirely when empty, so no placeholder claims.
- **Lightbox**: clicking the dimmed backdrop around the media now closes the viewer (Escape/arrow keys unchanged).
- **Sample seed content for the new sections** (all `is_sample = true`): 4 more short-form sample videos (vertical cutdowns), 3 `thumbnail_design` sample projects with picsum covers, and 4 new media services (Photography, Social media & short-form, Live streaming, Church & convention coverage) so the capabilities list covers the full PRD set.
...
### Changed
- **2026-09-10 — media homepage plan and composed page rebuilt** around the new blocks (see Added); `/media/services` composed page adds a tools grid between process and contact (seeded `source: services`); media hero gains a subtle availability ping-dot + location line under the CTAs (settings-driven; renders nothing when unset).
...
### Fixed
- **Tech portfolio "Featured Projects" honesty (2026-09-10, later).** Sample (`is_sample`) projects are now filtered out of the Featured Projects section on `/tech-portfolio` and `/tech`, so it renders the same "Portfolio coming soon / Case studies are being prepared. Check GitHub for the latest code." card + "View My GitHub" button as the live covenant-media.netlify.app site instead of three placeholder cards. Real projects entered in the CMS (`is_sample = false`) appear there automatically once published. The `/tech/projects` catalog route is unchanged.
- **`/media/work` filter integrity.** The page previously rendered an unfiltered `project_grid` block above the filterable `ProjectCatalog`, so `?form=short_form` / `?category=…` still showed every project above the filtered list. The composed CMS page now carries a `page_header` instead and the structural plan is empty (mirroring `/blog` — the catalog is the content; the blocks-plans test was updated to allow that exception). Filtered URLs now show only matching projects.
- **Mobile hero previews.** With the default "floating" hero variant, mobile rendered no video previews at all (the strip container carried a `hidden` class). Both variants now show the snap strip below the hero copy on small screens.
- **`fill-content.ts` `newPages` blocks now honour a `media` attachment** (used to attach the founder portrait to the Media about sections); previously the insert loop hardcoded `media: []`.
