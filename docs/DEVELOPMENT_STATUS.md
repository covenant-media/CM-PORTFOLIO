...
_Last updated: 2026-09-14 (CMS dashboard round: the admin is now an Overview plus four section hubs — Media portfolio, Tech portfolio, Main website and System — each with its own counters, tool grid and boards for the hero preview, client stories, event photography, the studio About block and the social accounts. Sidebar section labels link their hub, and the paste-a-link video importer was repaired: the detection columns (`source`, `source_id`, `embed_url`, `poster_url`, `metadata_state`) are now registry fields, so a pasted link can actually be stored. Verified end to end through real form POSTs, 104 tests green. Public surfaces were not touched in this round. Previous entry — 2026-09-12 (Deployment round: made the CMS work on a hosted platform — the schema now ships with the server bundle and self-migrates, a first-owner bootstrap creates the owner from `ADMIN_EMAIL`/`ADMIN_PASSWORD` on an empty database, the driver and `/admin` explain a missing database instead of crashing, and sign-in reports an unreachable database on the form. On top of the Windows PGlite double-writer fix, the rebuilt database, the two owner accounts, the credential-hint removal, the `/icon` fix and the Turbopack root pin. Tech Portfolio and the main brand site untouched.)_
...
- **The CMS is organised as an Overview plus four section hubs** (`CMS_SECTIONS`): Overview (`/admin`), Media portfolio (`/admin/media-portfolio`), Tech portfolio (`/admin/tech-portfolio`), Main website (`/admin/main-website`), System (`/admin/system`). Each hub carries the section's live public links, four counters from `dashboardCounts()`, a grid of that section's editors and the boards below; every module belongs to exactly one section through `CmsModuleDef.group`, and the sidebar prints each module once under its section label, which links the hub.
- **Boards on the hubs**: hero text + hero-preview toggle per short-form video (`hero-on`/`hero-off`, ordered by `sort_order`), paste-a-link boxes for short-form and long-form, per-video client-story editors with the derived attribution shown before saving, event-photography upload into the media library, the About-the-studio portrait/bio block, the social-accounts board with verified/published state, enquiries and the audit trail.
- **Client stories are derived, never invented** (`src/lib/media/story.ts`): the story name wins, then the video's `client` column, then the title — "Wedding Highlight — Mary and Real" is attributed as *Wedding clients: Mary and Real*, a piece that names nobody stays unnamed, and a story is only rendered when a real quote was typed.
