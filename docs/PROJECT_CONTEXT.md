...
- **Block-based page composition** — A block registry (`src/components/blocks/index.tsx`) renders ~29 section types (incl. media showcases: featured work, short-form rail, photo gallery, thumbnail wall) (hero variants, grids, video walls, testimonial walls, timelines, pricing tables, skill matrix, resume, contact CTA, rich text, etc.). Pages either use CMS-composed blocks or fall back to a structural plan in `src/lib/cms/page-plans.ts`.
