...
- A fresh seed creates exactly one owner with the local demo default `covenant@example.test / covenant-demo-2026` (overridable via `ADMIN_EMAIL` / `ADMIN_PASSWORD`), and the seed only runs that block while the account table is empty. No credentials are ever rendered on the sign-in page, so a reachable `/admin/login` discloses nothing about the accounts it accepts.
