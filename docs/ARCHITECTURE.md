...
- Three public surfaces share a `CmsPage` component which wraps content in `ExperienceShell`:
  ...
  - `/media`: the one Media Portfolio page — renders `MediaPortfolioPage` directly (not via `CmsPage`), with its own `MediaHeader` and in-page anchors (`home`, `work`, `shortform`, `photography`, `services`, `process`, `stories`, `about`, `contact`).
  - `/media/pricing`: the only other standalone media screen, rendering `MediaPricingPage` (package groups, quote factors, process recap, contact CTAs).
  - `/media/work`, `/media/services`, `/media/about`, `/media/contact`: **308 redirects** to the matching `/media#section` anchor, declared in `next.config.mjs` -> `redirects()`. The route folders are gone; old links and CMS navigation still land somewhere useful. `/media/work/[slug]` remains a real route (CMS project detail view), though the one-pager does not link to it.
...
- Site-specific components (`SiteHeader`, `SiteFooter`, `Logo`, `WorkCard`, `CatalogView`, `ProjectDetailView`, `ArticleView`, `ExperienceShell`, and the `Media*` family listed above) live in `src/components/site/`. Form primitives live in `src/components/forms/` (`PublicForm`, `SelectMenu`).
...
- Public video previews are muted floating posters. Clicking opens the details card, whose player mounts **muted and autoplaying** — the one start browsers always allow — and then switches the sound on by commanding the player that is already running (`enablejsapi` + the embed's `postMessage` command channel, retried as the frame comes up). The frame's source is frozen once it exists, so changing the sound never rebuilds the embed: no reload, no lost position, and no second click on the platform's own play button. Platforms with no scriptable mute (TikTok, Facebook, Instagram) get their embed as-is and their player decides.
...
- `src/lib/media/`
  ...
  - `embed.ts` — Media-Portfolio-only capability layer over `video.ts`: a `CAPABILITIES` matrix (mutedAutoplay / controllableMute / posterDerivable / embeddable / aspect per platform), `resolvePlayable(url, thumbnail?)`, `buildEmbed(playable, mode, {loop, controls})` which only appends mute parameters the platform actually supports, `posterCandidates(playable, given?)` for the maxres → hq → mq fallback chain, and `platformLabel`. New platforms are added by extending the matrix, not by rewriting callers.
  - `platform-metadata.ts` — optional enrichment from platform oEmbed endpoints (YouTube/Vimeo/TikTok), returning provider, title, author, thumbnail and, only where the platform publishes it, duration. Facebook and Instagram need an app token and return `null` by design. `enrichItem`/`enrichMedia` only issue a request for an entry that opts in and is missing something, own values always win, and any failure degrades to the record so a render never depends on a third party.
  - `sample-portfolio.ts` — the Media Portfolio's content: eight published items (four long-form, four short-form with real YouTube ids and posters), six photography records, five hero roles, eight services, five process steps, six client stories, four pricing groups and the studio/social facts. Types and field names mirror the `project` / `media_video` / `media_asset` / `testimonial` / `pricing_package` columns so a CMS/DB loader can replace the arrays without touching components. No database access.
- `src/lib/seo/`
...
| `ExperienceShell` | Applies `theme-{surface}`, renders grain/atmosphere, `SiteHeader`, `SiteFooter`, `SiteBehaviours`, `LightboxHost`; handles maintenance mode. Persistent controls are per-surface: the media screens render `MediaBackToTop` themselves. |
...
Media-Portfolio-specific components (not shared with the main or tech surfaces): `site/MediaPortfolioPage.tsx` (the page, including the two-column hero), `site/MediaHeader.tsx` (page and sub-page variants), `site/MediaHeroVideo.tsx` (the framed short-form reel: 9:16 stage, five-second dwell, the waiting piece mounted and playing beside the card, one masked strip carrying the roll, prev/next controls inside the rim, dots), `site/MediaTools.tsx` (the static "Tools I use" band) and `ui/ToolMark.tsx` (the real tool icons it serves from `public/images/tools/`), `site/MediaStats.tsx` (the studio figures band), `site/MediaSocialButtons.tsx` (the hero social row), `site/MediaTicker.tsx` (one seamless loop used by both long-form rows, the short-form rail and the client stories), `site/MediaCards.tsx` (`MediaPreviewCard` and `MediaDetailsCard`), `site/MediaGalleries.tsx` (long-form rows, short-form rail, ratio-true photo wall, testimonial rail), `site/MediaCatalogGrid.tsx` + `site/MediaCatalogView.tsx` + `site/MediaCatalogScreen.tsx` (the catalogs, their category filters and their enquiry form), `site/MediaVideoPlayer.tsx` (the player: muted mount → command-channel sound, native `<video>` for uploaded files, outbound link over the poster when a host cannot be embedded), `site/MediaRoleLine.tsx`, `site/MediaBackToTop.tsx`. Two structural decisions are worth knowing before touching them: `MediaDetailsCard` is rendered through `createPortal(..., document.body)` and animates on transform alone (`cm-panel-in`, no opacity), because a fixed overlay inside an animated ancestor is clipped to that ancestor and an opacity fade on a backdrop is a fade of the page behind it; and `MediaHeroVideo` runs one masked strip under the card (`--cm-ring` / `--cm-gap` / `--cm-peek`), which is what lets the outgoing piece leave through the left of the frame while the incoming piece, already mounted and playing, rolls in from the peek beside it. `ui/Lightbox.tsx` gained an opt-in media presentation for the photography viewer (solid backdrop, intrinsic-ratio framing); the other surfaces keep the original markup and behaviour, and `ui/MediaTile`, `ui/Motion`, `ui/Marquee`, `forms/PublicForm`, `CmsPage`, `SiteHeader` and `blocks/media.tsx` are untouched. They deliberately do not modify `ui/MediaTile`, `ui/Motion`, `ui/Lightbox`, `ui/Marquee`, `forms/PublicForm`, `CmsPage`, `SiteHeader` or `blocks/media.tsx`, which keep serving the other surfaces exactly as before.
